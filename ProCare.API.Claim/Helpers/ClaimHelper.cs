...
namespace ProCare.API.Claims.Helpers
{
    public class ClaimHelper
    {
        ...
        private static List<string> IncludeIfMissingRetroLICSFields = new List<string> { "DI", "IN" };



        public async Task<string> GetSoftwareIdByAnsiBin(string binNumber, string processorControlNumber)
        {
            Dictionary<string, string> setting = null;
            CommonApiHelper commonApiHelper = new CommonApiHelper(Environment.GetEnvironmentVariable("Global.CommonApiBaseUrl"), Environment.GetEnvironmentVariable("Global.CommonApiUsername"), Environment.GetEnvironmentVariable("Global.CommonApiPassword"));

            setting = await commonApiHelper.GetSetting("Global.ConnectionStrings").ConfigureAwait(false);

            DataHelper = new SQLHelper(setting["CST"].ToString());
            return (new ClaimDataAccess(DataHelper)).GetSoftwareIdByAnsiBin(binNumber, processorControlNumber);
        }
        ...
        public bool UseTricareReversalExclusions()
        {
            return ReversalRequest.Header.ProcessorControlNumber.Equals("A4") && ReversalRequest.Insurance.GroupId.Equals("DODA");
        }
        ...
        public void MapClaimRequestRetroLics()
        {
            Ncpdp.CurrentTransaction.RetroLics.BBPFrom = ClaimRequest.RetroLics.BBPFrom;
            Ncpdp.CurrentTransaction.RetroLics.BBRFrom = ClaimRequest.RetroLics.BBRFrom;
            Ncpdp.CurrentTransaction.RetroLics.PhaNeutral = ClaimRequest.RetroLics.PhaNeutral;
            Ncpdp.CurrentTransaction.RetroLics.PlnNeutral = ClaimRequest.RetroLics.PlnNeutral;
            Ncpdp.CurrentTransaction.RetroLics.RUnitAwp = ClaimRequest.RetroLics.RUnitAwp;
        }
        ...
        public Transmission MapClaimReversalRequestClaimToNcpdp(ClaimReversalRequest request)
        {
            ...
            if (!UseTricareReversalExclusions())
            {
        ...
        public T MapClaimSubmissionResponseFromNcpdp<T>(TransmissionResponse response)
        ...
        public void MapEligibilityInsurance()
        {
            ...
            Ncpdp.Insurance.GroupId = EligibilityRequest.Insurance.GroupId;
            Ncpdp.Insurance.PatientRelationshipCode = EligibilityRequest.Insurance.PatientRelationshipCode;
        }
        ...
        private void MapPricingOtherAmountClaimed()
        {
            ...
            //Add a 0 DispFee to Retro LICS claims if one was not submitted
            if (ClaimRequest.Header.SoftwareId.ToUpper().Equals("RETRO"))
            {
                foreach (string includeIfMissingField in IncludeIfMissingRetroLICSFields)
                {
                    if (!Ncpdp.CurrentTransaction.Pricing.OtherAmountClaimeds.Any(
                        x => x.OtherAmountClaimedSubmittedQualifier.Equals(includeIfMissingField)))
                    {
                        Ncpdp.CurrentTransaction.Pricing.OtherAmountClaimeds.Add(new NCPDP.Telecom.Request.OtherAmountClaimed
                        {
                            OtherAmountClaimedSubmittedQualifier = includeIfMissingField,
                            OtherAmountClaimedSubmitted = 0m
                        });
                    }
                }
            }
        ...
        public void MapReversalRequestClaim()
        {
            ...
            if (!UseTricareReversalExclusions())
            {
                Ncpdp.CurrentTransaction.Claim.QuantityDispensed = ReversalRequest.Claim.QuantityDispensed;
                Ncpdp.CurrentTransaction.Claim.DaysSupply = ReversalRequest.Claim.DaysSupply;
                Ncpdp.CurrentTransaction.Claim.CompoundCode = ReversalRequest.Claim.CompoundCode;
                Ncpdp.CurrentTransaction.Claim.DispenseAsWritten = ReversalRequest.Claim.DispenseAsWritten;
                Ncpdp.CurrentTransaction.Claim.PrescriptionOriginCode = ReversalRequest.Claim.PrescriptionOriginCode;
            }
        }
        ...
        public void MapReversalRequestInsurance()
        {
            if (ReversalRequest.Insurance != null)
            {
                ...
                Ncpdp.Insurance.PlanId = ReversalRequest.Insurance.PlanId;
                Ncpdp.Insurance.FacilityId = ReversalRequest.Insurance.FacilityId;

                if(!UseTricareReversalExclusions())
                {
                    Ncpdp.Insurance.PersonCode = ReversalRequest.Insurance.PersonCode;
                    Ncpdp.Insurance.PatientRelationshipCode = ReversalRequest.Insurance.PatientRelationshipCode;
                    Ncpdp.Insurance.CardholderFirstName = ReversalRequest.Insurance.CardholderFirstName;
                    Ncpdp.Insurance.CardholderLastName = ReversalRequest.Insurance.CardholderLastName;
                }
            }
        }
        ...
        {
            if (Ncpdp.Header.TransactionCode != TransactionCode.Reversal ||
                !UseTricareReversalExclusions())
            {
                Ncpdp.Patient.Residence = PatientResidence.Home;
            }
            
            if (Ncpdp.Header.TransactionCode == TransactionCode.Reversal &&
                string.IsNullOrWhiteSpace(ReversalRequest.Claim.OtherCoverageCode))
            {
                Ncpdp.CurrentTransaction.Claim.OtherCoverageCode = "0";
            }
        }
